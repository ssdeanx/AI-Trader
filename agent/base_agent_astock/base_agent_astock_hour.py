# -*- coding: utf-8 -*-
"""
BaseAgentAStock_Hour class - A-share hourly trading agent
...
class BaseAgentAStock_Hour(BaseAgentAStock):
    ...
    async def run_trading_session(self, today_date: str) -> None:
        ...
        if self.model is None:
            raise RuntimeError("Model is not initialized. Call initialize() before running trading session.")
        ...
        while current_step < self.max_steps:
            ...
            try:
                ...
                if isinstance(agent_response, str) and STOP_SIGNAL in agent_response:
                    ...
                    self._log_message(log_file, [{"role": "assistant", "content": content}])
                    break
    ...
    def _check_daily_completeness(self, trading_times: List[str], date: str) -> Dict[str, Any]:
        ...
        if not result["is_complete"]:
            ...
            print(f"   Expected time points: {len(expected_times)} total {sorted(expected_times)}")
            print(f"   Actual time points: {len(found_times)} total {sorted(found_times)}")

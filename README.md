...
[🚀 Quick Start](#-quick-start) • [📈 Performance Analysis](#-performance-analysis) • [🛠️ Configuration Guide](#-configuration-guide)
...
### 🖥️ Windows Quick Start (PowerShell)

Windows users: we provide PowerShell scripts and a master orchestrator to make setup and runs easy. We recommend installing UV (https://astral.sh/uv) and running from PowerShell (use `.ilename.ps1`).

- Run the full end-to-end workflow (data → services → agent → cache → web UI background):

```powershell
# Example: complete A-share workflow using Gemini (updates config to use Gemini if configured)
.\Run-AITrader.ps1 -Action all -Market astock -Config configs/astock_config.json
```
...
# Run agent only
.\Run-AITrader.ps1 -Action agent -Config configs/astock_config.json
...
Notes:
- `-Action all` now runs data preparation (unless `-SkipData`), starts MCP services (unless `-SkipServices`), runs the trading agent, regenerates the cache, and launches the web UI in the background on port 8888.
- PowerShell scripts are in `./scripts/` and mirror the shell scripts; use the `.ps1` versions on Windows.
- Script logs are written to `scripts/logs/` for easier debugging.
...
[![GitHub stars](https://img.shields.io/github/stars/ssdeanx/AI-Trader?style=social)](https://github.com/HKUDS/AI-Trader)
[![GitHub forks](https://img.shields.io/github/forks/ssdeanx/AI-Trader?style=social)](https://github.com/HKUDS/AI-Trader)
...
<div align="center">
  <a href="https://star-history.com/#HKUDS/AI-Trader&Date">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=ssdeanx/AI-Trader&type=Date&theme=dark" />
      <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=ssdeanx/AI-Trader&type=Date" />
      <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=ssdeanx/AI-Trader&type=Date" style="border-radius: 15px; box-shadow: 0 0 30px rgba(0, 217, 255, 0.3);" />
    </picture>
  </a>
</div>
...
<p align="center">
  ...
  <img src="https://visitor-badge.laobi.icu/badge?page_id=ssdeanx.AI-Trader&style=for-the-badge&color=00d4ff" alt="Views">
</p>
